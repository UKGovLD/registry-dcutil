...
VOL=`aws ec2 describe-volumes --filters "Name=tag:Name,Values=DCUtilVol" | jq .Volumes[0].VolumeId | sed -e 's/"\(.*\)"/\1/'`
if [[ -z $VOL ]]; then
...
SNAPSHOT=`aws ec2 create-snapshot --volume-id $VOL --description "Incremental backup for DCUtilVol"  | jq .SnapshotId | sed -e 's/"\(.*\)"/\1/'` &&
aws ec2 create-tags --resources $SNAPSHOT --tags "Key=Name,Value=DCUtilVol-backup" &&
